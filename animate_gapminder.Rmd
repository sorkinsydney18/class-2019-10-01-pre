...
```{r make_plot}
gapminder %>% 
  mutate(ccontinent = factor(continent, levels = c("Americas", "Africa", "Asia"))) %>% 
  
  #use %in% to say in vector 
  filter(continent %in% c("Americas","Africa","Asia")) %>% 
  ggplot(aes(x = gdpPercap, y = lifeExp, size = pop)) +
  geom_point(aes(color = country), show.legend = FALSE, alpha = .7) +
  
  #this creates labels for breaks 
  scale_x_log10(labels = c("$1,000","$10,000"), breaks = c(1000, 10000)) + 
  
  #create range of y axis and breaks 
  scale_y_continuous(limits = c(20,85), breaks = c(40,60,80)) +
  facet_wrap(~continent) +
  
  #creates animation by year
  transition_time(year) +
  
  #{frame_time} allows the year to tick forward with animation 
    labs(title = "The World Gets Better Every Year: {frame_time}",
         x = "GDP Per Capita",
         y = "Life Expectancy") +
  
  #creates gapmider colors for each country
